...
const MapProvider: React.FC<{ context: MapContextType; children?: React.ReactNode }> = ({ children, context }) => {
...
const MapContainer: React.FC<{ mapRef: MapRef; children?: React.ReactNode }> = ({ children, mapRef }) => {
  return <div ref={mapRef} style={{ width: '100%', height: '100%' }} children={children} />;
};
...
> = ({ tokenOrCallback, language = 'en', mapkit, map, mapRef, ...props }) => {
  ...
  return <MapkitProvider tokenOrCallback={tokenOrCallback} language={language} children={mapBox} />;
};
